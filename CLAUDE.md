...
Driving beads with `bh`, triaging with `bv`, and the dolt-remote push are documented in
[AGENTS.md](AGENTS.md#beads-workflow-beadhive). That copy is the single source of truth: it
sits inside `bv`'s marker comments, which is where `bv` looks, so duplicating it here would
drift.
