#!/usr/bin/env python3
"""Incremental extractor for Discogs data with change tracking."""
...
from common.processing_state import (
    ProcessingState,
...
change_stats = {
    "artists": {"created": 0, "updated": 0, "deleted": 0, "unchanged": 0},
    "labels": {"created": 0, "updated": 0, "deleted": 0, "unchanged": 0},
    "masters": {"created": 0, "updated": 0, "deleted": 0, "unchanged": 0},
    "releases": {"created": 0, "updated": 0, "deleted": 0, "unchanged": 0},
}
...
def get_health_data() -> dict[str, Any]:
    ...
    return {
        "status": "healthy",
        "service": "incremental-extractor",
        "current_task": current_task,
        "progress": current_progress,
        "extraction_progress": extraction_progress.copy(),
        "change_stats": change_stats.copy(),
...
class IncrementalExtractor:
    """Extractor with incremental processing and change tracking."""
    ...
    def __init__(
        ...
        self.changes_published: int = 0
        self.current_file_url: str | None = None
        self.current_file_checksum: str | None = None
    ...
    def __enter__(self) -> "IncrementalExtractor":
        retry_count = 0
        while retry_count < MAX_RETRIES:
            try:
                ...
                # Create a special queue for changes
                changes_queue_name = f"discogsography-changes-{self.data_type}"
                ...
                # Declare changes queue
                self.amqp_channel.queue_declare(
                    auto_delete=False, durable=True, queue=changes_queue_name
                )
                self.amqp_channel.queue_bind(
                    exchange=AMQP_EXCHANGE,
                    queue=changes_queue_name,
                    routing_key=f"{self.data_type}.changes",
                )
    ...
    async def extract_async(self) -> None:
        """Async extraction with concurrent record processing and change tracking."""
        logger.info(
            f"🚀 Starting incremental extraction of {self.data_type} from {self.input_file}"
        )
        self.start_time = datetime.now()
        ...
        metadata = {
            ...
            "file_checksum": self.current_file_checksum,
            "file_size": self.current_file_size,
        }
        self.processing_run_id = self.state_tracker.start_processing_run(
            self.data_type, metadata
        )

        # Update processing state to 'processing'
        state = self.state_tracker.get_processing_state(
            self.data_type
        ) or ProcessingState(data_type=self.data_type)
        state.processing_status = "processing"
        state.last_file_url = self.current_file_url
        state.last_file_checksum = self.current_file_checksum
        state.last_file_size = self.current_file_size
        self.state_tracker.update_processing_state(state)
        ...
        try:
            ...
            deleted_count = self.state_tracker.detect_deleted_records(
                self.data_type, self.processing_run_id, self.seen_record_ids
            )
            if deleted_count > 0:
                change_stats[self.data_type]["deleted"] += deleted_count
            ...
            error_state = self.state_tracker.get_processing_state(self.data_type)
            if error_state is not None:
                error_state.processing_status = "error"
                error_state.error_message = str(e)
                self.state_tracker.update_processing_state(error_state)
            raise
        finally:
            self.end_time = datetime.now()

            # Complete the processing run
            stats = change_stats[self.data_type]
            self.state_tracker.complete_processing_run(
                self.processing_run_id,
                records_processed=self.total_count,
            ...
            final_state = self.state_tracker.get_processing_state(self.data_type)
            if final_state is not None:
                final_state.processing_status = "idle"
                final_state.last_processed_at = datetime.now(UTC)
                final_state.total_records_processed = self.total_count
                final_state.error_message = None
                self.state_tracker.update_processing_state(final_state)
            ...
            logger.info(
                f"✅ Incremental Extraction Complete: {self.total_count:,} {self.data_type} processed "
                f"(created: {stats['created']:,}, updated: {stats['updated']:,}, "
                f"deleted: {stats['deleted']:,}, unchanged: {stats['unchanged']:,}) "
                f"in {elapsed} (avg {final_tps:.1f} records/sec)"
            )
            logger.info(
                f"📊 Published {self.changes_published:,} changes to downstream services"
            )
    ...
    async def _process_records_async(self) -> None:
        """Process records from the queue asynchronously with change detection."""
        ...
        while True:
            try:
                ...
                # Process the record with change detection
    ...
    async def _process_record_async(self, data: dict[str, Any]) -> None:
        """Process a single record asynchronously with change detection."""
        try:
            # Extract record ID
            record_id = data.get("id", "unknown")
            if record_id == "unknown":
                logger.warning(f"⚠️ Record without ID in {self.data_type}: {data}")
                return
            ...
            # Compute record hash (excluding the sha256 field itself)
            record_data_for_hash = {
                k: v for k, v in normalized_data.items() if k != "sha256"
            }
            record_hash = self.state_tracker.compute_record_hash(record_data_for_hash)

            # Update record state and check for changes
            assert self.processing_run_id is not None, "Processing run ID should be set"  # nosec B101
            change = self.state_tracker.update_record_state(
                self.data_type, record_id, record_hash, self.processing_run_id
            )

            if change:
                # Record changed - add to pending messages
                change_stats[self.data_type][change.change_type] += 1

                # Add change metadata to the record
                normalized_data["_change_type"] = change.change_type
                normalized_data["_processing_run_id"] = str(self.processing_run_id)
                ...
                # Add to pending messages with special routing
                with self.pending_messages_lock:
                    self.pending_messages.append(
                        {
                            "_routing_key": f"{self.data_type}.changes",
                            **change_notification,
                        }
                    )
            else:
                # No change
                change_stats[self.data_type]["unchanged"] += 1
                logger.debug(
                    f"⏩ Skipped {self.data_type[:-1]} ID={record_id} (no changes)"
                )
                # Don't publish unchanged records
                should_flush = False
    ...
    def __queue_record(
        ...
        if self.total_count % self.progress_log_interval == 0:
            ...
            stats = change_stats[self.data_type]
            logger.info(
                f"📊 Incremental Progress: {self.total_count:,} {self.data_type} processed "
                f"(created: {stats['created']:,}, updated: {stats['updated']:,}, "
                f"unchanged: {stats['unchanged']:,}, errors: {self.error_count:,}, "
        ...
        try:
            ...
            for message_data in messages_to_send:
                try:
                    ...
                    routing_key = message_data.pop("_routing_key", self.data_type)
                    ...
                    if published is False:
                        ...
                        if routing_key.endswith(".changes"):
            ...
            logger.debug(
                f"✅ Flushed {regular_messages} data messages and {change_notifications} "
                f"change notifications to AMQP exchange"
            )
...
async def process_file_incremental(
    discogs_data_file: str,
    config: ExtractorConfig,
    state_tracker: ProcessingStateTracker,
) -> None:
    ...
    try:
        ...
        data_type = discogs_data_file.split("_")[2].split(".")[0]

        # Check processing state to see if we need to process this file
        state = state_tracker.get_processing_state(data_type)
        file_path = Path(config.discogs_root, discogs_data_file)

        # Get file metadata
        file_url = f"s3://discogs-data-dumps/data/{discogs_data_file}"
        ...
        if file_path.exists():
            ...
            file_checksum = hash_obj.hexdigest()
        else:
            file_checksum = None

        # Check if this file has already been processed
        if state and state.last_file_checksum == file_checksum:
            checksum_display = file_checksum[:8] + "..." if file_checksum else "None"
            logger.info(
                f"⏩ Skipping {discogs_data_file} - already processed "
                f"(checksum: {checksum_display})"
            )
            return

        logger.info(
            f"🆕 Processing {discogs_data_file} - new or updated file "
            f"(checksum: {file_checksum[:8] if file_checksum else 'unknown'}...)"
        )

        extractor = IncrementalExtractor(discogs_data_file, config, state_tracker)
        extractor.current_file_url = file_url
        extractor.current_file_checksum = file_checksum
        extractor.current_file_size = file_size
...
async def process_discogs_data_incremental(
    config: ExtractorConfig, state_tracker: ProcessingStateTracker
) -> bool:
    """Process Discogs data files incrementally. Returns True if successful."""
    global extraction_progress, last_extraction_time, change_stats
    ...
    async def process_with_semaphore(file: str) -> None:
        async with semaphore:
            if shutdown_requested:
                return
            await process_file_incremental(file, config, state_tracker)
    ...
    async def progress_reporter() -> None:
        ...
        while not shutdown_requested:
            ...
            total_changes = sum(
                sum(stats.values()) - stats["unchanged"]
                for stats in change_stats.values()
            )
            logger.info(
                f"📊 Incremental Progress: {total} total records, {total_changes} changes "
...
async def periodic_check_loop_incremental(
    config: ExtractorConfig, state_tracker: ProcessingStateTracker
) -> None:
    ...
    logger.info(
        f"🔄 Starting incremental periodic check loop (interval: {periodic_check_days} days)"
    )
    ...
    while True:
        ...
        try:
            success = await process_discogs_data_incremental(config, state_tracker)
...
async def main_async() -> None:
    """Main async function with incremental processing and periodic checks."""
    # Set up signal handlers for graceful shutdown
    signal.signal(signal.SIGINT, signal_handler)
    signal.signal(signal.SIGTERM, signal_handler)

    setup_logging(
        "incremental-extractor", log_file=Path("/logs/incremental-extractor.log")
    )
    ...
    # Check if PostgreSQL is configured for incremental processing
    if not all(
        [
        ...
        logger.error(
            "❌ PostgreSQL configuration is required for incremental processing. "
            "Please set POSTGRES_ADDRESS, POSTGRES_USERNAME, POSTGRES_PASSWORD, "
            "and POSTGRES_DATABASE environment variables."
        )
        sys.exit(1)

    # Build PostgreSQL connection string
    assert config.postgres_address is not None, "PostgreSQL address is required"  # nosec B101
    if ":" in config.postgres_address:
        host, port = config.postgres_address.split(":", 1)
    else:
        host = config.postgres_address
        port = "5432"

    postgres_connection = f"postgresql://{config.postgres_username}:{config.postgres_password}@{host}:{port}/{config.postgres_database}"

    # Initialize state tracker
    state_tracker = ProcessingStateTracker(postgres_connection)
    ...
    logger.info("🚀 Starting Discogs incremental data extractor with change tracking")
    ...
    logger.info("📥 Starting initial incremental data processing...")
    initial_success = await process_discogs_data_incremental(config, state_tracker)

    if not initial_success:
        logger.error("❌ Initial data processing failed")
        sys.exit(1)

    logger.info("✅ Initial incremental data processing completed successfully")

    # Start periodic check loop
    if not shutdown_requested:
        logger.info("🔄 Starting periodic check service...")
        await periodic_check_loop_incremental(config, state_tracker)

    # Stop health server
    health_server.stop()
    logger.info("✅ Incremental extractor service shutdown complete")
...
def main() -> None:
    ...
    try:
        ...
        logger.info("⚠️ Incremental extractor interrupted by user")
    except Exception as e:
        logger.error(f"❌ Incremental extractor failed: {e}")
        sys.exit(1)
