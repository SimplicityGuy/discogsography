...
class TestAdminConfigEndpoint:
    """Tests for PUT /api/admin/config/{key}."""

    def test_set_known_key_success(
        self,
        test_client: TestClient,
        auth_headers: dict[str, str],
    ) -> None:
        response = test_client.put(
            "/api/admin/config/discogs_consumer_key",
            headers=auth_headers,
            json={"value": "mykey"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["key"] == "discogs_consumer_key"
        assert data["updated"] is True

    def test_set_unknown_key_400(
        self,
        test_client: TestClient,
        auth_headers: dict[str, str],
    ) -> None:
        response = test_client.put(
            "/api/admin/config/unknown_key",
            headers=auth_headers,
            json={"value": "something"},
        )
        assert response.status_code == 400

    def test_set_empty_value_400(
        self,
        test_client: TestClient,
        auth_headers: dict[str, str],
    ) -> None:
    ...
    def test_set_consumer_secret_success(
        self,
        test_client: TestClient,
        auth_headers: dict[str, str],
    ) -> None:
        response = test_client.put(
            "/api/admin/config/discogs_consumer_secret",
            headers=auth_headers,
            json={"value": "mysecret"},
        )
        assert response.status_code == 200
