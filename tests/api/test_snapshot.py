...
class TestSaveSnapshot:
    ...
    def test_save_snapshot_success(self, test_client: TestClient) -> None:
        ...
        response = test_client.post("/api/snapshot", json=body)
    ...
    def test_save_snapshot_empty_nodes_422(self, test_client: TestClient) -> None:
        body = {"nodes": [], "center": {"id": "1", "type": "artist"}}
        response = test_client.post("/api/snapshot", json=body)
        assert response.status_code == 422

    def test_save_snapshot_too_many_nodes(self, test_client: TestClient) -> None:
    ...
    def test_save_snapshot_missing_fields(self, test_client: TestClient) -> None:
        response = test_client.post("/api/snapshot", json={"nodes": [{"id": "1", "type": "artist"}]})
        assert response.status_code == 422
...
class TestRestoreSnapshot:
    ...
    def test_restore_snapshot_success(self, test_client: TestClient) -> None:
        # First save a snapshot
        body = {
            "nodes": [{"id": "1", "type": "artist"}],
            "center": {"id": "1", "type": "artist"},
        }
        save_response = test_client.post("/api/snapshot", json=body)
