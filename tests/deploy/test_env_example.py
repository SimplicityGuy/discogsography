...
production code ever read that name — api/config.py and api/setup.py only
...
def test_documents_encryption_master_key_instead() -> None:
    ...
    assert re.search(r"^ENCRYPTION_MASTER_KEY=", text, re.MULTILINE), (
        ".env.example must set ENCRYPTION_MASTER_KEY (the var name api/config.py actually reads)"
    )
...
def test_env_example_var_names_are_all_read_by_config_or_documented_elsewhere() -> None:
    ...
    api/config.py's get_secret() convention actually consults — i.e. it
    isn't itself a new dead var.
    """
    config_text = (REPO_ROOT / "api" / "config.py").read_text()
