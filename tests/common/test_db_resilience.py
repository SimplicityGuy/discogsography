...
class TestCircuitBreakerEdgeCases:
    ...
    async def test_call_async_lock_initialized_lazily(self) -> None:
        """Test that asyncio.Lock is lazily initialized on first async use (safe for Python 3.12+)."""
        import asyncio

        config = CircuitBreakerConfig(name="TestBreaker")
        breaker = CircuitBreaker(config)

        # Lock is None before first async call (lazy initialization)
...
class TestAsyncLazyLockInit:
    ...
    async def test_on_success_async_uses_lazy_lock(self) -> None:
        """Test _on_success_async lazily creates and uses asyncio.Lock."""
        import asyncio

        config = CircuitBreakerConfig(name="TestBreaker")
        breaker = CircuitBreaker(config)

        # Lock is None before first use (lazy initialization)
        assert breaker._async_lock is None

        # Call _on_success_async directly — creates lock lazily
    ...
    async def test_on_failure_async_uses_lazy_lock(self) -> None:
        """Test _on_failure_async lazily creates and uses asyncio.Lock."""
        import asyncio
        ...
        # Call _on_failure_async directly — creates lock lazily
