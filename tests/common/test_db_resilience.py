...
class TestCircuitBreakerEdgeCases:
    ...
    async def test_call_async_lock_initialized_eagerly(self) -> None:
        """Test that asyncio.Lock is eagerly initialized at construction time."""
        import asyncio

        config = CircuitBreakerConfig(name="TestBreaker")
        breaker = CircuitBreaker(config)

        # Lock is created eagerly in __init__ (safe since Python 3.10+)
        ...
        # Lock still exists after async call
        assert isinstance(breaker._async_lock, asyncio.Lock)
