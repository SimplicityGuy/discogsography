...
# ---------------------------------------------------------------------------
# DiggerConfig tests — DiggerConfig lives in common/config.py so its coverage
# is collected here under the common service coverage config.
# ---------------------------------------------------------------------------
...
def _set_digger_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """Set all required environment variables for DiggerConfig."""
    for key, val in _DIGGER_REQUIRED.items():
        monkeypatch.setenv(key, val)


class TestDiggerConfigFromEnv:
    """Tests for DiggerConfig.from_env() — covers all branches in common/config.py."""
    ...
    def test_from_env_defaults(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Non-required fields use documented defaults when env vars are absent."""
        _set_digger_env(monkeypatch)
        for var in (
    ...
    def test_missing_postgres_username_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_missing_postgres_database_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_missing_redis_host_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_missing_multiple_vars_lists_all(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """ValueError lists every missing required var when several are absent."""
        for key in _DIGGER_REQUIRED:
    ...
    # ----- Invalid int fallback branches -----

    def test_rate_budget_invalid_string_uses_default(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Non-integer DIGGER_RATE_BUDGET_PER_HOUR falls back to 600."""
        _set_digger_env(monkeypatch)
        monkeypatch.setenv("DIGGER_RATE_BUDGET_PER_HOUR", "not-a-number")

        cfg = DiggerConfig.from_env()

        assert cfg.rate_budget_per_hour == 600

    def test_cb_window_invalid_string_uses_default(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Non-integer DIGGER_CB_WINDOW_SECONDS falls back to 300."""
        _set_digger_env(monkeypatch)
        monkeypatch.setenv("DIGGER_CB_WINDOW_SECONDS", "bad")

        cfg = DiggerConfig.from_env()

        assert cfg.circuit_breaker_window_seconds == 300

    def test_cb_failure_pct_invalid_string_uses_default(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Non-integer DIGGER_CB_FAILURE_PCT falls back to 30."""
        _set_digger_env(monkeypatch)
        monkeypatch.setenv("DIGGER_CB_FAILURE_PCT", "bad")

        cfg = DiggerConfig.from_env()

        assert cfg.circuit_breaker_failure_pct == 30

    def test_cb_cooldown_invalid_string_uses_default(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Non-integer DIGGER_CB_COOLDOWN_SECONDS falls back to 1800."""
        _set_digger_env(monkeypatch)
        monkeypatch.setenv("DIGGER_CB_COOLDOWN_SECONDS", "bad")
    ...
    def test_postgres_connstr_format(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_redis_url_format(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # ----- get_secret (_FILE variant) integration -----

    def test_postgres_password_from_file(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        """POSTGRES_PASSWORD_FILE is honoured — exercises get_secret _FILE branch."""
        _set_digger_env(monkeypatch)
        secret_file = tmp_path / "pg_pass.txt"
        secret_file.write_text("file-pg-pass\n")
        monkeypatch.setenv("POSTGRES_PASSWORD_FILE", str(secret_file))
        monkeypatch.delenv("POSTGRES_PASSWORD", raising=False)

        cfg = DiggerConfig.from_env()

        assert cfg.postgres_password == "file-pg-pass"

    def test_postgres_username_from_file(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        """POSTGRES_USERNAME_FILE is honoured — exercises get_secret _FILE branch."""
        _set_digger_env(monkeypatch)
        secret_file = tmp_path / "pg_user.txt"
        secret_file.write_text("file-pg-user\n")
        monkeypatch.setenv("POSTGRES_USERNAME_FILE", str(secret_file))
        monkeypatch.delenv("POSTGRES_USERNAME", raising=False)

        cfg = DiggerConfig.from_env()

        assert cfg.postgres_username == "file-pg-user"

    # ----- Frozen dataclass -----

    def test_config_is_frozen(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """DiggerConfig is a frozen dataclass — attribute assignment must raise."""
        _set_digger_env(monkeypatch)
        cfg = DiggerConfig.from_env()
