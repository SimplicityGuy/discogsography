...
- **FLAT package layout (like graphinator/tableinator), NOT a nested `digger/digger/`.** The `digger/` directory IS the package: `digger/__init__.py` (re-exports `main`), `digger/main.py`, `digger/health.py`, `digger/metrics.py`, `digger/scraper/…`. In `digger/pyproject.toml` use `packages = ["."]` and `[project.scripts] digger = "digger:main"`. Do NOT exclude `digger/` from root mypy and do NOT add it to root pytest `pythonpath` — a nested same-name package breaks `explicit_package_bases` (mypy "source file found twice"); the flat layout is type-checked by root `mypy .` like graphinator. Tests import `from digger.health import …`.
