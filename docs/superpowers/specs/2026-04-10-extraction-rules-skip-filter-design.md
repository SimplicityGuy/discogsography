...
| Filter condition type | Regex (`remove_matching`) | Numeric-only detection needs pattern matching; reuses existing regex infrastructure |
...
struct CompiledFilterCondition {
    field: String,
...
- For each filter condition, resolves the parent path (e.g., `genres`) and the array key (e.g., `genre`)
- Navigates to the array in the JSON Value, removes elements matching the regex
- Mutates the record in-place
- Returns a `Vec<FilterAction>` describing what was removed (for logging)
...
- `test_yaml_parsing_with_skip_and_filters` — full YAML round-trip
...
| File | Change |
|------|--------|
| `extractor/extraction-rules.yaml` | Add `skip_records` and `filters` sections |
| `extractor/src/rules.rs` | New types, parsing, `should_skip_record()`, `apply_filters()`, report changes |
...
| `tests/extractor/` | Rust unit tests for skip/filter logic |
