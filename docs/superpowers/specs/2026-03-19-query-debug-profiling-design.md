# Query Debug Logging & Cypher Profiling

## Problem

The API service executes Neo4j Cypher and PostgreSQL SQL queries across many files with no visibility into what queries are running or how they perform. Developers debugging slow queries or unexpected results must add ad-hoc logging, then remove it. There is no way to profile Cypher query execution plans without modifying code.

## Solution

Add opt-in query debug logging and Cypher profiling to the API service, controlled by existing `LOG_LEVEL` and a new `CYPHER_PROFILING` environment variable.

## Behavior Matrix

| LOG_LEVEL | CYPHER_PROFILING | Console log (structlog JSON)       | `/logs/profiling.log`                              |
|-----------|-----------------|------------------------------------|----------------------------------------------------|
| INFO+     | any             | No query logging                   | Nothing                                            |
| DEBUG     | unset/false     | Full query + params (Cypher & SQL) | Nothing                                            |
| DEBUG     | true            | Full query + params (Cypher & SQL) | Full query + params + PROFILE/EXPLAIN summary      |

Both conditions (DEBUG **and** CYPHER_PROFILING=true) must be met for profiling. The PROFILE/EXPLAIN summary never appears in the console log — only in the dedicated profiling log file.
...
- `is_cypher_profiling() -> bool` — returns True only when `is_debug()` AND `CYPHER_PROFILING` env var is `"true"` (case-insensitive).
...
- `log_profile_result(cypher, params, summary)` — writes query + params + `summary.profile['args']['string-representation']` to profiling log.
- `log_explain_result(cypher, params, summary, original_error)` — writes query + params + `summary.plan['args']['string-representation']` + error details to profiling log.
- `execute_sql(cursor, query, params)` — executes `await cursor.execute(query, params)`, logging the query at DEBUG level beforehand. This is the standard SQL execution path with optional observability. Handles both plain string queries and `psycopg.sql.Composable` objects — for Composable, renders to string via `query.as_string(cursor)` for the log output. The `params` argument accepts `list`, `tuple`, `dict`, or `None`.
...
- At end of `setup_logging()`: if DEBUG + CYPHER_PROFILING, log a startup warning: `"⚠️ Cypher profiling enabled — PROFILE prefix will be added to all Cypher queries"`.

#### `common/__init__.py`

- Export `execute_sql`, `is_debug`, `is_cypher_profiling` from `common/query_debug`.
...
### PROFILE result (successful query)
...
### EXPLAIN result (after query failure)
...
| `CYPHER_PROFILING` | true, false | (unset = false) | New. When true AND LOG_LEVEL=DEBUG, adds PROFILE to Cypher queries and writes results to profiling log. |
...
- PROFILE adds execution overhead — it should never be enabled in production.
- The profiling log file is in `/logs/` alongside existing service logs, governed by the same volume mounts and access controls.

## Testing

- Unit tests for `is_debug()`, `is_cypher_profiling()` with mocked env/log level.
