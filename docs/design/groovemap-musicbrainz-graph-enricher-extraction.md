# `groovemap-music/musicbrainz-graph-enricher` extraction evidence

Date: 2026-08-27

Destination: <https://github.com/groovemap-music/musicbrainz-graph-enricher>
...
`wt/bead/issue/discogsography-2kpm.16` at
`69d90758ddae646f8a54eb458f95ea340d144858`. `git filter-repo` retained
`brainzgraphinator/`, its owned tests, applicable MusicBrainz/resilience design documents,
and the license. Exact arguments are committed in the destination's
`docs/extraction.md`.

The filter retained 68 relevant commits. Destination commit
`6d3f139136428a4d0479bddd2251b3e6444d5799` established the standalone repository, for
69 commits total. No tags were copied or created. The original monorepo `main` remains
...
- one service wheel and non-root container image own MusicBrainz-to-Neo4j enrichment;
- `groovemap-runtime[neo4j,rabbitmq]` version `0.1.0` is pinned to immutable private
...
- `just check`: passed;
- 106 focused mocked-service tests: passed with 96% total coverage;
- Ruff and strict mypy over 4 source modules: passed;
...
- gitleaks history scan: 68 commits scanned, no leaks;
...
- GitHub Actions Source CI run `33049085932`: passed;
